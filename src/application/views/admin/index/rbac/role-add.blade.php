        ...
        <form action="" method="post" class="form form-horizontal" id="form-admin-role-add">
            ...
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="" placeholder="" id="roleName" name="roleName" datatype="*4-16" nullmsg="角色名称不能为空">
                </div>
            </div>
            <div class="row cl">
                    ...
                    @if($list)
                        @foreach($list as $v)
                            <dl class="permission-list">
                                <dt>
                                    <label>
                                        <input type="checkbox" value="{{$v['fa']['suf_code']}}" name="permission[{{$v['fa']['pre_code']}}][]" id="user-Character-0">
                                        {{$v['fa']['title']}}</label>
                                </dt>
                                <dd>
                                    <dl class="cl permission-list2">
                                        <dd>
                                            @if(@$v['ch'])
                                                @foreach($v['ch'] as $cv)
                                                <label class="">
                                                    <input type="checkbox" value="{{$cv['suf_code']}}" name="permission[{{$v['fa']['pre_code']}}][]" id="user-Character-0-0-0">
                                                    {{$cv['title']}}</label>
                                                @endforeach
                                            @endif
                                        </dd>
